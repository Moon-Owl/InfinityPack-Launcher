...
public class CheckBoxNodeTreeSample {
	...
	public static void main(String args[]) {
		...
		NamedVector<CheckBoxNode> accessVector = new NamedVector<CheckBoxNode>(
				"Accessibility", accessibilityOptions);
		NamedVector<CheckBoxNode> browseVector = new NamedVector<CheckBoxNode>(
		...
		Image im = Toolkit.getDefaultToolkit()
				.getImage(
						new File(workingDirectory, "splash_logo.png")
								.getAbsolutePath());
		JList list = new JList(new Image[] { im, im });

		list.setLayoutOrientation(JList.HORIZONTAL_WRAP);
		Image tek = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "tekkit_unselected.png")
						.getAbsolutePath());
		Image tekh = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "tekkit_hover.png")
						.getAbsolutePath());
		Image teks = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "tekkit_selected.png")
						.getAbsolutePath());
		Image tec = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "technic_unselected.png")
						.getAbsolutePath());
		Image tech = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "technic_hover.png")
						.getAbsolutePath());
		Image tecs = Toolkit.getDefaultToolkit().getImage(
				new File(workingDirectory, "technic_selected.png")
						.getAbsolutePath());

		FancyButton b1 = new FancyButton(new ImageIcon(tek),
				new ImageIcon(teks), new ImageIcon(tekh));
		FancyButton b2 = new FancyButton(new ImageIcon(tec),
				new ImageIcon(tecs), new ImageIcon(tech));
