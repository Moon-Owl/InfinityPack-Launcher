...
public class ModPackListYML {
	...
	public static final File ORIGINAL_PROPERTIES = new File(
			GameUpdater.cacheDir, "launcher.properties");
	...
	private static final File MODPACKS_YML_FILE = new File(GameUpdater.workDir,
			MODPACKS_YML);
	...
	public static boolean setModPack(String modPack, String modPackLabel,
		...
		if (!ignoreCheck) {
			...
			if (!modPacks.containsKey(modPack)) {
				...
				Util.log("ModPack '%s' not in '%s' file.", modPackLabel,
						MODPACKS_YML);
				return false;
			}
		}
	...
	public static void downloadModPackResources(String name, String label,
		...
		if (downloadFileList.size() > 0
				&& DownloadUtils.downloadFiles(downloadFileList, 30,
						TimeUnit.SECONDS) != downloadFileList.size()) {
			Util.log("Could not download all resources for modpack '%s'.",
					label);
		}
	}
	...
	public static void loadModpackLogos() {
		for (String modPack : modpackMap.keySet()) {
			...
			if (!modPackLogo.exists())
				continue;
			modpackLogoList.put(modPack, new ImageIcon(Toolkit
					.getDefaultToolkit()
